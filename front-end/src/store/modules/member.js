...
const actions = {
    ...
    getMemberInfo({ commit, state }) {
        ...
        axios
            ...
            .then(res => {
                ...
                state.member.picturePath =
                    "/api/member/profile/img/" + res.data.profile.picturePath;
            })
            .catch(() => {
                localStorage.removeItem("codebook-bearer");
                alert(
                    "로그인 기한이 만료되어 로그아웃 처리 되었습니다.\n다시 로그인 해주세요."
                );
            });
        commit;
    },
    signUp({ commit, dispatch, state }, userData) {
        dispatch("emailDuplicateCheck", userData.email).then(() => {
    ...
    profileUpdate({ commit, dispatch, state }, profileForm) {
        ...
        axios
            .put("/api/member/profile", profileForm, config)
            .then(() => {
    ...
    profileImgUpdate({ commit, state }, file) {
        ...
        axios
            .put("/api/member/profile/img", formData, config)
            .then(() => {
